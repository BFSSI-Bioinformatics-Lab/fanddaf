...
from ..models import Product, Barcode, NutritionFacts, Ingredients, ProductImage, Batch, Store
...
class ProductSetupForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['source_batch'].queryset = self._active_or_current_queryset(Batch, 'source_batch_id')
        self.fields['store'].queryset = self._active_or_current_queryset(Store, 'store_id')

    def _active_or_current_queryset(self, model, instance_field_name):
        """Active rows, plus the instance's current value even if it's since been deactivated"""
        current_id = getattr(self.instance, instance_field_name, None)
        if current_id:
            return model.objects.filter(Q(is_active=True) | Q(pk=current_id))
